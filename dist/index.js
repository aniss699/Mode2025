...
router5.get("/feed", async (req, res) => {
  try {
    ...
    const rawAnnouncements = await db.select().from(announcements).where(and3(...whereConditions)).orderBy(desc4(announcements.created_at)).limit(limitNum + 5).catch((err) => {
...
import { eq as eq13, and as and9, or as or2, desc as desc6 } from "drizzle-orm";
...
router10.get("/conversations/:id/messages", async (req, res) => {
  try {
    ...
    }).from(messages).leftJoin(users, eq13(messages.sender_id, users.id)).where(eq13(messages.conversation_id, conversationId)).orderBy(desc6(messages.created_at)).limit(limit).offset(offset);
    res.json({
  ...
  try {
    ...
    const existingConversation = await db.select().from(conversations).where(
      and9(
        or2(
          and9(
            eq13(conversations.participant1_id, participant1_id),
            eq13(conversations.participant2_id, participant2_id)
          ),
          and9(
            eq13(conversations.participant1_id, participant2_id),
            eq13(conversations.participant2_id, participant1_id)
          )
        ),
        mission_id ? eq13(conversations.mission_id, mission_id) : void 0
      )
    ).limit(1);
...
router10.post("/messages", async (req, res) => {
  try {
    ...
    const conversation = await db.select().from(conversations).where(
      and9(
        eq13(conversations.id, conversation_id),
        or2(
          eq13(conversations.participant1_id, sender_id),
          eq13(conversations.participant2_id, sender_id)
        )
      )
    ).limit(1);
    if (conversation.length === 0) {
    ...
    await db.update(conversations).set({ last_message_at: /* @__PURE__ */ new Date() }).where(eq13(conversations.id, conversation_id));
...
var messaging_default = router10;
...
import { eq as eq14, and as and10, or as or3 } from "drizzle-orm";
...
var WebSocketManager = class {
  ...
  async handleSendMessage(userId, message) {
    try {
      ...
      await db.update(conversations).set({ last_message_at: /* @__PURE__ */ new Date() }).where(eq14(conversations.id, conversationId));
...
router13.get("/providers/available", async (req, res) => {
  try {
...
router13.get("/demo-providers", async (req, res) => {
  try {
...
router13.get("/demo-projects", async (req, res) => {
  try {
    ...
    }).from(users).leftJoin(users, eq17(users.id, users.id));
...
router13.get("/demo-bids", async (req, res) => {
  try {
    const bidsWithInfo = await db4.select({
      ...
      provider_email: users.email,
      provider_profile: users.profile_data
    }).from(bids).leftJoin(users, eq17(bids.project_id, users.id)).leftJoin(users, eq17(bids.provider_id, users.id));
...
router13.get("/provider/:id", async (req, res) => {
  try {
    const providerId = parseInt(req.params.id);
    const provider = await db4.select().from(users).where(eq17(users.id, providerId)).limit(1);
    ...
    const providerBids = await db4.select({
      ...
      project_title: users.name,
      project_budget: users.email
    }).from(bids).leftJoin(users, eq17(bids.project_id, users.id)).where(eq17(bids.provider_id, providerId));
    res.json({
      provider: {
        id: providerData.id,
...
router13.get("/ai-analysis-demo", async (req, res) => {
  try {
...
router13.use("/missions", authMiddleware, missions_default);
router13.use("/bids", authMiddleware, bids_default);
router13.use("/teams", authMiddleware, open_teams_default);
router13.use("/feed", feed_routes_default);
router13.use("/favorites", authMiddleware, favorites_routes_default);
router13.use("/reviews", authMiddleware, reviews_default);
router13.use("/contracts", authMiddleware, contracts_default);
router13.use("/files", authMiddleware, files_default);
router13.use("/", messaging_default);
router13.use("/", notifications_default);
router13.use("/", user_settings_default);
var api_routes_default = router13;
...
router14.get("/experiments", async (req, res) => {
  try {
    const experiments = [
      {
        id: "exp-001",
...
router14.post("/events", async (req, res) => {
  try {
...
router14.get("/performance-metrics", async (req, res) => {
  try {
...
router14.post("/clear-cache", async (req, res) => {
  try {
...
router14.get("/business-metrics", async (req, res) => {
  try {
...
router14.get("/alerts", async (req, res) => {
  try {
    const alerts = [
      {
        id: "alert-001",
...
var ai_monitoring_routes_default = router14;
...
router15.post("/assistant-suggestions", async (req, res) => {
  try {
...
var ai_suggestions_routes_default = router15;
...
var ai_missions_routes_default = router16;
...
router17.post("/ai/quick-analysis", async (req, res) => {
  try {
...
router17.post("/ai/price-analysis", async (req, res) => {
  try {
...
var ai_quick_analysis_default = router17;

// server/routes/ai-diagnostic-routes.ts
import { Router as Router12 } from "express";
var router18 = Router12();
router18.get("/diagnostic", async (req, res) => {
  try {
...
var ai_diagnostic_routes_default = router18;
...
// server/routes/ai-learning-routes.ts
var router19 = Router13();
router19.post("/analyze-patterns", async (req, res) => {
  try {
...
var ai_learning_routes_default = router19;
...
router20.post("/create-project", async (req, res) => {
  try {
...
var team_routes_default = router20;
...
router21.post("/subscriptions/reverse", async (req, res) => {
  try {
...
router21.post("/group-requests", async (req, res) => {
  try {
...
router21.get("/group-requests/interest", async (req, res) => {
  try {
    const { location, category } = req.query;
    const count = await db5.select().from(missions).where(eq18(missions.category, category)).then((missions4) => missions4.filter((m) => {
      try {
...
router21.post("/team-building-projects", async (req, res) => {
  try {
...
router21.post("/ia-human-jobs", async (req, res) => {
  try {
...
router21.get("/opportunities/live-slots", async (req, res) => {
  try {
    const { category, minRating, maxPrice, location } = req.query;
    const providers = await db5.select().from(users).where(eq18(users.role, "PRO")).limit(10);
...
var services_routes_default = router21;
