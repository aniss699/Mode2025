...
-- 6. Trigger automatique pour synchroniser missions → announcements
CREATE OR REPLACE FUNCTION sync_mission_to_announcement()
RETURNS TRIGGER AS $$
BEGIN
  -- Appeler la fonction d'upsert avec les données de la mission
  PERFORM upsert_announcement(
    NEW.id,
    NEW.title,
    NEW.description,
    COALESCE(NEW.excerpt, LEFT(NEW.description, 200)),
    COALESCE(NEW.category, 'general'),
    COALESCE(NEW.tags, '{}'),
    CONCAT(ROUND(COALESCE(NEW.budget_value_cents, 0) / 100), '€'),
    NEW.budget_value_cents,
    COALESCE(NEW.currency, 'EUR'),
    COALESCE((NEW.location_data->>'city')::TEXT, 'Remote'),
    (NEW.location_data->>'city')::TEXT,
    COALESCE((NEW.location_data->>'country')::TEXT, 'France'),
    NEW.user_id,
    'Client',
    CASE WHEN NEW.status = 'open' THEN 'active' ELSE 'inactive' END,
    COALESCE(NEW.urgency, 'medium'),
    NEW.deadline,
    0.8
  );
  RETURN NEW;
...
DROP TRIGGER IF EXISTS missions_to_announcements_trigger ON missions;
CREATE TRIGGER missions_to_announcements_trigger
  AFTER INSERT OR UPDATE ON missions
  FOR EACH ROW
  EXECUTE FUNCTION sync_mission_to_announcement();

COMMENT ON TRIGGER missions_to_announcements_trigger ON missions IS 'Synchronise automatiquement missions vers announcements';
