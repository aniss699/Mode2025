...
export const dataApi = {
  ...
  /**
   * Récupère le feed (même source que marketplace : missions)
   */
  async getFeed(cursor?: string, limit = 10): Promise<{ items: AnnouncementView[]; hasMore: boolean }> {
    try {
      // Utiliser la même API que le marketplace
      const { missions } = await this.getMissions();
      
      // Convertir les missions en format feed (AnnouncementView)
      const feedItems = missions.slice(0, limit).map((mission: MissionView) => ({
        id: mission.id,
        title: mission.title,
        description: mission.description,
        category: mission.category,
        budgetMin: mission.budget,
        budgetMax: mission.budget,
        city: mission.location,
        userId: parseInt(mission.clientId || '0'),
        tags: [],
        sponsored: false,
        qualityScore: 0.8
      }));

      console.log('✅ Feed normalisé depuis missions:', feedItems.length);
      return {
        items: feedItems,
        hasMore: missions.length > limit
      };
    } catch (error) {
