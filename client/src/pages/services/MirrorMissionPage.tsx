...
export default function MirrorMissionPage() {
  ...
  const onSubmit = async (data: MirrorMissionFormData) => {
    ...
    try {
      // Créer la mission miroir en base de données
      const response = await fetch('/api/missions', {
        ...
        body: JSON.stringify({
          ...
          budget: data.budgetPerProvider.toString(),
          location: data.location,
          urgency: 'medium',
          requirements: JSON.stringify({
            mirrorMode: true,
            budgetPerProvider: data.budgetPerProvider,
            allowSameProvider: data.allowSameProvider,
      ...
      if (!response.ok) {
        throw new Error('Erreur lors de la création de la mission miroir');
      }
      ...
      setTimeout(() => {
        window.location.href = `/missions/${result.id}`;
      }, 1500);
    } catch (error) {
      toast({
        title: "Erreur",
